...
public class SubscriptionBusinessLogic extends AbstractQueryParamBusinessLogic implements ISubscriptionResource {
	 ...
	 * Cause: Issue to Init. statically InternalDatabase because the behavior of static initialization is unpredictable
	 *        at tomcat initialization time.
	 * Workaround: Remove of static init. at static class member scope and add explicit call in method that need access to InternalDatabase
	 *
	 */
    //private static InternalDatabase SUBSCRIPTIONS_DB = InternalDatabase.getInstance();
    private static InternalDatabase SUBSCRIPTIONS_DB = null;
...
/**** DEBUG_START ****/
			Logger.getLogger(SubscriptionBusinessLogic.class.getName()).log(Level.INFO, "SubscriptionBusinessLogic.addSubscription: ID = " + id);
/**** DEBUG_END ****/
...
/**** DEBUG_START ****/
			Logger.getLogger(SubscriptionBusinessLogic.class.getName()).log(Level.INFO, "Before call of webservice: subscriptionURLAbsolute = " + subscriptionURLAbsolute);
/**** DEBUG_END ****/

            VoidHolder voidHolder = EPCISWebServiceClient.subscribe(subscribe);

/**** DEBUG_START ****/
			Logger.getLogger(SubscriptionBusinessLogic.class.getName()).log(Level.INFO, "After call of webservice: EPCISWebServiceClient.subscribe(subscribe)");
/**** DEBUG_END ****/
    ...
    private String generateAlias(String id) {
    	SUBSCRIPTIONS_DB = InternalDatabase.getInstance();

...
/**** DEBUG_START ****/
                LOG.debug("Handled query value: " + paramValue);
/**** DEBUG_END ****/
            }
            try {
    ...
    private Calendar parseAsCalendar(Object queryParamValue, String queryParamName)
        ...
        if (queryParamValue instanceof Calendar) {
			 ...
			 * Cause: Issue to get the correct date calendar when the value is a long date/time value in millis
			 *        at tomcat initialization time.
			 * Workaround: Check and try to convert long to timestamp and calendar object
			 *
			 */
    		try {
    			// Test if a date is a long time millis value
        		long lValue = Long.parseLong(date);

        		// If no exception lValue is a long value then do the convertion to a Calendar Object
           		Timestamp ts = new Timestamp(lValue);

/**** DEBUG_START ****/
                LOG.debug("Trying to convert long to calendar (" + date + ") as Timestamp " + ts.toString() + " as date/time");
/**** DEBUG_END ****/

            	try {
            		cal = TimeParser.convert(ts);

/**** DEBUG_START ****/
                	LOG.debug("After trying to convert long to calendar = " + cal.toString() + " and Timestamp = " + ts.toString());
/**** DEBUG_END ****/

            		return cal;
            	} catch (Exception e) {
            	    // Do nothing and try to continue
            	}
    		} catch (Exception e) {
            	// Do nothing and try to continue calling the original method
    		}
	...
	/** Utils functions
	 *
	 * boolean isLong(String string): Test if a string is a long numeric value
	 *
	 */
